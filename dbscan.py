...
def _region_query(m, point_id, eps):
    # n_points = m.shape[1]  #when executed as main
    n_points = len(m[0])   #when used as a module
    seeds = []
    for i in range(0, n_points):
        # if _eps_neighborhood(m[:, point_id], m[:, i], eps):  #when executed as main
...
def dbscan(m, eps, min_points):
    cluster_id = 1
    # n_points = m.shape[1] #when executing in main
    n_points = len(m[1])  #when used as a module
    classifications = [UNCLASSIFIED] * n_points
    for point_id in range(0, n_points):
        # point = m[:, point_id] #when executing as main
